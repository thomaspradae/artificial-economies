...
- [x] Resource Island Phase 3 full comparison table exists at `outputs/resource_island_phase3_full/mind_comparison.csv` with 20 v1 rows across Q-learning, DQN, PPO, fixed independent-DQN, and centralized-critic for four pressure-tested Resource Island institutions.
- [x] Thesis-facing Resource Island comparison table is now the main v1-backed `outputs/resource_island_phase3_full/mind_comparison.csv`; the earlier distinct-mind workaround and v0 comparison framing are obsolete after the fixed independent-DQN and v1 ladder reruns.
...
- [x] Resource Island v1 Phase 3 full ladder outputs exist for DQN, PPO, fixed independent-DQN, and centralized-critic at `outputs/resource_island_v1_*_full/`; each has 80 by-seed rows, 4 aggregate rows, 20 seeds per institution, matching v1 pressure protocol settings, and no blank/NaN/inf values.
- [x] Resource Island v1 Phase 3 full comparison table exists at `outputs/resource_island_v1_phase3_full/mind_comparison.csv` with 20 rows across Q-learning, DQN, PPO, fixed independent-DQN, and centralized-critic for four v1 institution variants.
- [x] Resource Island v1 strict-local trade-radius ablation exists at `outputs/resource_island_v1_strict_radius_full/` with `trade_radius=1`, `steps=40000`, `n_seeds=20`, four institution variants, 80 by-seed rows, 4 aggregate rows, and no blank/NaN/inf values.
...
- [x] Canonical five-world protocol comparability passes after the Resource Island v1 ladder rebuild: `outputs/cross_world_synthesis/protocol_comparability_report.json` marks Pricing Arena, Resource Island, Auction House, Public Goods, and Labor Market comparable for cross-mind capability claims.
...
- [x] `run_claim_audit_suite.py`: reusable cross-world claim-audit runner exists and writes paired institution effects, benchmark-gap robustness, metric-conflict/activation warnings, trace/full-run alignment checks, and a Markdown safe-claim report across Pricing Arena, Resource Island v1, Auction House, Public Goods, and Labor Market.
...
- [x] P.6 full comparison: final thesis-facing Resource Island v1 full cross-mind table has 20 rows across Q-learning, DQN, PPO, fixed independent-DQN, and centralized-critic.
...
- [x] All-world visualization bundle exists at `explorer/public/data/all-world-ladder/summary.json`, covering Pricing Arena, Resource Island v1, Auction House, Public Goods, and Labor Market across Q-learning, DQN, PPO, fixed independent-DQN, and centralized-critic where the full-run tables support them.
...
- [x] Resource Island v0 cross-mind diagnostic: stronger neural/MARL minds improved survival and welfare in the earlier Resource Island setup, but they did so without learning successful trade. In that n=20 table, Q-learning was the only mind with nonzero successful trades on `none` (`trade_count_mean=0.2939`), while DQN, PPO, independent-DQN, and centralized-critic all had `trade_count_mean=0.0`; this motivated the v1 pressure redesign.
- [x] Initial cross-world synthesis: capability effects are real in both worlds, but their form is world-specific. In Pricing Arena, stronger learners can exploit a price cap after sufficient training, but the dedicated audit shows the effect is budget-dependent and seed-heterogeneous rather than visible in short traces as a simple quantity channel. In Resource Island, stronger learners improve survival/welfare while failing to activate the trade/property institutions, so the non-pricing result should be framed as a coordination/exploration limitation of the current world setup rather than a clean institution-effect comparison.
- [x] Resource Island v1 cross-mind answer: after adding contested resources, unequal trades, specialization pressure, and the full neural/MARL ladder, the trade result changes materially. PPO and centralized-critic activate the trade/reputation channel strongly, DQN and fixed independent-DQN trade less but remain distinct, and `trade_price_controls` binds by construction and suppresses successful trade across minds. The earlier v0 neural non-trade result should therefore be treated as a world-design/training diagnostic, not the final Resource Island capability result.
- [x] Full-data five-world synthesis: `outputs/cross_world_synthesis/` now combines comparable full ladder outputs for Pricing Arena, Resource Island v1, Auction House, Public Goods, and Labor Market. The protocol-comparability report passes for all five worlds, and `outputs/publication_inference/` has been rebuilt with all five worlds included in confirmatory inference.
- [x] Cross-world claim-strength audit layer: `run_claim_audit_suite.py` now detects paired-seed robustness classes, metric conflicts, activation confirmations, and trace/full-run sign mismatches across the five full-run worlds. Current outputs flag the Pricing Arena DQN/independent-DQN/centralized-critic price-cap trace/full-run profit mismatches, Public Goods reward/accounting warnings, Labor Market welfare-vs-truthfulness/stability tradeoffs, Auction revenue-vs-efficiency/regret tradeoffs, and Resource Island v1 activation confirmations.
