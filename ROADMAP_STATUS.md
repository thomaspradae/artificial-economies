...
- [x] Exploitability tests pass: `test_exploitability.py` ran 8 tests, all passing, including one-deviator N-firm exploitability smoke coverage.
...
- [x] Resource Island tests pass: `test_resource_island.py` ran 25 tests, all passing, covering metrics, benchmarks, movement/collision, gathering/starvation, posted-offer trading, trade-radius behavior, institutions, Q-learning integration, inventory-aware observations, contested layouts, unequal trades, property-pressure counters, specialization rewards, and smoke output.
...
- [x] Full unittest discovery passes: 164 tests, all passing.
- [x] Python compile check passes for the legacy v0 module, experiment scripts, new `core/`, `worlds/`, `minds/`, `institutions/`, Resource Island modules, Auction House modules/training runner, Public Goods modules/training/group-size runners, Labor Market modules/training runner, deep-RL/MARL modules, validation runners, combined-table builder, and all test files.
...
- [x] Resource Island Phase 3 full comparison table exists at `outputs/resource_island_phase3_full/mind_comparison.csv` with 20 rows across Q-learning, DQN, PPO, fixed independent-DQN, and centralized-critic for four pressure-tested Resource Island institutions.
- [x] Thesis-facing Resource Island comparison table is now the main pressure-tested `outputs/resource_island_phase3_full/mind_comparison.csv`; the earlier distinct-mind workaround is obsolete after the fixed independent-DQN ladder rerun.
...
- [x] Resource Island pressure smoke output exists at `outputs/resource_island_v1_pressure_smoke/` with `steps=500`, `n_seeds=3`, contested resources, complementary specialization preferences, 2-for-1 unequal trade offers, pressure-start inventories/positions, Q-learning agents, summary CSVs, and manifest.
- [x] Resource Island pressure validation output exists at `outputs/resource_island_v1_validation/` with `steps=5000`, `n_seeds=5`, contested resources, complementary specialization preferences, 2-for-1 unequal trade offers, pressure-start inventories/positions, Q-learning agents, summary CSVs, and manifest.
- [x] Resource Island full pressure-tested output exists at `outputs/resource_island_v1_full/` with `steps=40000`, `n_seeds=20`, four institution variants, 80 by-seed rows, 4 aggregate rows, contested resources, unequal trades, property-pressure diagnostics, and no blank/NaN/inf values.
- [x] Resource Island Phase 3 full ladder outputs exist for DQN, PPO, fixed independent-DQN, and centralized-critic at `outputs/resource_island_v1_*_full/`; each has 80 by-seed rows, 4 aggregate rows, 20 seeds per institution, matching pressure-tested protocol settings, and no blank/NaN/inf values.
- [x] Resource Island Phase 3 full comparison table exists at `outputs/resource_island_v1_phase3_full/mind_comparison.csv` with 20 rows across Q-learning, DQN, PPO, fixed independent-DQN, and centralized-critic for four pressure-tested institution variants.
- [x] Resource Island strict-local trade-radius ablation exists at `outputs/resource_island_v1_strict_radius_full/` with `trade_radius=1`, `steps=40000`, `n_seeds=20`, four institution variants, 80 by-seed rows, 4 aggregate rows, and no blank/NaN/inf values.
...
- [x] Known-answer sanity-check output exists at `outputs/known_answer_sanity_checks_current/` with 19/19 passing checks across Pricing Arena N-firm static Nash best responses (`n=2,3,4,5`), Auction House second-price truthfulness and first-price shading (`n=2,3`), Public Goods free-rider/cooperative brackets (`n=2,4,8`), Labor Market fixed deferred-acceptance/stability/strategy-proofness cases, and Resource Island oracle/greedy gather brackets.
- [x] General mechanism-trace outputs exist at `outputs/mechanism_traces_local_full/` with `steps=1000`, all five worlds, all five thesis-facing minds, `n_agents=2`, 30,000 per-step trace rows, and 510 algebraic decomposition rows; `run_mechanism_traces.py` supports configurable `n_agents` and covers Pricing price/margin/quantity, Auction bid/value/regret/efficiency, Public Goods contribution/extraction/stock, Labor truthfulness/stability/welfare, and Resource Island trade/property/specialization channels.
- [x] Pricing Arena price-cap contradiction audit exists at `outputs/pricing_quantity_margin_audit/` with paired full-run seed deltas, fresh 20-seed 1000-step checks for all thesis minds, and a DQN budget ladder at 1000/5000/10000/40000 steps. The audit resolves the short-trace/full-run contradiction as a training-budget and seed-heterogeneity effect: DQN is negative at 1000 steps, mixed at 5000, mostly positive by 10000, and positive in the existing 40k paired full audit.
- [x] Cross-world claim-audit suite exists at `outputs/claim_audit_suite/` with 1,025 paired/benchmark audit rows, 31 metric-conflict or activation rows, 20 trace/full-run alignment rows, and an automatically generated claim-risk report. The suite applies the Pricing-Arena contradiction standard across all five worlds using existing full-run outputs: paired seed effects, benchmark gaps, metric-conflict detection, activation checks, and trace/full-run sign agreement where paired traces exist.
- [x] Literature theory-scout pipeline exists at `tools/theory_scout/` with OpenAlex, Semantic Scholar, arXiv, Unpaywall/PDF, text-extraction, strict paper-card, coverage review, manual PDF queue, Scholar-comparison worksheet, ranking, gap-table, world-obligation bridge, and theory-to-code audit modules; `test_theory_scout.py` passes and `literature/` contains an OpenAlex-backed first cache, strict paper cards, `novelty_gap_table.csv`, `theory_obligations.md`, `theory_coverage.md`, `manual_pdf_queue.csv`, `scholar_comparison_worksheet.csv`, and `theory_code_audit.md`.
- [x] Citation/claim-support audit exists at `tools/theory_scout/citation_audit.py` and is wired into `python -m tools.theory_scout.cli citation-audit`; `test_citation_audit.py` ran 4 tests, all passing, and the current paper audit wrote `paper/citation_audit.csv`, `paper/citation_audit.md`, and `paper/main_citation_patches.md` with explicit citation, artifact, and manual-review queues.
- [x] Preliminary cross-world synthesis outputs exist at `outputs/cross_world_synthesis_smoke/` with all five worlds represented, a capability-tier synthesis table, a monotonicity report, and a shared capability-ladder figure. This is retained as a smoke-scale diagnostic artifact, not the thesis-facing synthesis.
...
- [x] Canonical five-world protocol comparability passes after the Resource Island ladder rebuild: `outputs/cross_world_synthesis/protocol_comparability_report.json` marks Pricing Arena, Resource Island, Auction House, Public Goods, and Labor Market comparable for cross-mind capability claims.
- [x] Publication-inference outputs exist at `outputs/publication_inference/`, including `publication_inference_all.csv`, `thesis_confirmatory_results.csv`, and `publication_inference_summary.md`; the rebuilt summary applies Benjamini-Hochberg correction by world/effect/claim/metric family and includes all five worlds under comparable full-ladder protocols.

## Running Work

- [ ] Public Goods group-size full relaunch is running on `ofi1` at `outputs/public_goods_group_size_sweep_full_relaunch/`.
- [ ] Pricing Arena N=5 boundary/exploitability relaunch is running on `old1` at `outputs/pricing_nfirm_boundary_n5_relaunch/` and `outputs/pricing_nfirm_exploitability_n5_relaunch/`.
- [ ] Post-run diagnostics are queued via `scripts/run_post_run_diagnostics.sh` and should run after the active N-sensitive sweeps finish: known-answer checks, all-world N=2 mechanism traces, Pricing N=5 traces, and Public Goods N=8 traces.
...
- [x] `run_exploitability.py`: frozen-policy exploitability runner using the new pricing-arena training path, with adversary restarts, selected restart summaries, aggregate summaries, manifest, CI plots, and a one-deviator N-firm protocol that freezes N-1 incumbent policies while replacing one target firm with a fresh adversary.
- [x] `run_known_answer_sanity_checks.py`: theory-anchor validation runner exists and writes CSV/Markdown/manifest outputs for exact benchmark checks across worlds and N-agent/N-firm settings before learned results are interpreted.
- [x] `run_mechanism_traces.py`: reusable trace runner exists for all five worlds, all thesis-facing minds, and configurable N; it writes per-step JSON-payload trace rows plus a compact mechanism-decomposition table for the causal/accounting channels behind aggregate outcomes.
- [x] `run_pricing_quantity_margin_audit.py`: Pricing Arena price-cap contradiction audit runner exists and writes paired full-run seed deltas, fresh short-budget seed sweeps, an audited-mind budget ladder, and a thesis-facing diagnosis for distinguishing short-trace artifacts from full-run effects.
- [x] `run_claim_audit_suite.py`: reusable cross-world claim-audit runner exists and writes paired institution effects, benchmark-gap robustness, metric-conflict/activation warnings, trace/full-run alignment checks, and a Markdown safe-claim report across Pricing Arena, Resource Island, Auction House, Public Goods, and Labor Market.
...
- [x] `worlds/pricing_arena/env.py` supports N-firm pricing games through `len(config.quality) >= 2`, while preserving the duopoly default and p1/p2 compatibility fields for historical outputs.
- [x] `worlds/pricing_arena/benchmarks.py`: static benchmark wrapper exists.
- [x] `worlds/pricing_arena/benchmarks.py`: symmetric N-firm static benchmark references exist for non-duopoly Pricing Arena runs; the default duopoly path still delegates to the legacy benchmark helper for parity.
...
- [x] `worlds/resource_island/env.py`: `ResourceIslandWorld` subclasses `World` and implements bounded-grid movement, simultaneous collision resolution, inventory-aware local tabular observations, inventory, energy, gathering, starvation/death, resource regeneration, posted-offer trading, configurable trade radius, unequal trade units, specialization preferences, pressure-start inventories, property/trade diagnostic counters, property-opportunity counters, metrics, and render state.
...
- [x] `worlds/resource_island/training.py`: Resource Island training loop supports Q-learning, DQN, PPO, independent-DQN, and centralized-critic with tabular or fixed structured observations as appropriate, including activation counters in summaries.
- [x] `run_resource_island_smoke.py`: Resource Island smoke runner writes seed summaries, aggregate summaries, diagnostic counters, and manifest for `--mind q_learning`, `dqn`, `ppo`, `independent_dqn`, and `centralized_critic`; it supports contested/split layouts, activation-pressure presets, complementary specialization preferences, unequal trade units, and trade-acquisition rewards.
...
- [x] `run_public_goods_group_size_sweep.py`: Public Goods group-size sweep runner exists for `n_agents` x mind x institution experiments, writing by-seed summaries, aggregate summaries, scaling effects versus the smallest group size, and a manifest.
...
- [x] Phase 3 price-cap divergence diagnosis: exploitability falls under price cap across tested mind families, while collusion effects are metric-sensitive for higher-capability minds. DQN sits effectively at the cap under price cap (`avg_price_mean=3.999038`, `price_dispersion_mean=0.259025`) and earns materially higher profit than uncapped (`profit_total_mean=291.703303` vs. `236.345482`). The paired-seed contradiction audit confirms this is not a single aggregate artifact (`delta_profit_total_mean=55.357821`, 95% CI `[4.858770, 105.856871]`, positive seed share `0.85`), but it should be written as an emergent long-training/focal-cap effect with seed heterogeneity, not as a simple 1000-step quantity-margin trace. Fixed independent-DQN remains a distinct corroborating condition (`avg_price_mean=3.790038`, `profit_total_mean=272.954450`, `exploitability_mean=11.681067` under price cap).
...
- `core/metrics.py` now contains the current extracted formulas needed by the pricing multiseed/exploitability protocols plus Resource Island's generic survival, specialization, stability, robustness-under-shock, and resource-sustainability helpers.
...
- [x] P.5 Known-answer sanity report: run exact theorem/bracketing checks before interpreting learned-policy output, and explicitly label whether each check validates mechanics, benchmark construction, or learned behavior.
- [x] P.5 Mechanism trace report: for any headline result, generate per-step traces and metric decompositions so the claim shows the channel, not only the aggregate table.
...
- [x] `worlds/resource_island/trading.py`: distance helpers and simple one-shot trade matching support; no multi-round bargaining in the current Resource Island protocol.
...
- [x] P.6 full comparison: final thesis-facing Resource Island full cross-mind table has 20 rows across Q-learning, DQN, PPO, fixed independent-DQN, and centralized-critic.

Resource Island full-run note:

- The thesis-facing Resource Island configuration is the pressure-tested scarce-resource economy: contested resources, complementary specialization preferences, unequal trade offers, property-pressure counters, institution-block counters, and strict-local trade-radius ablations. This world is evaluated by whether the relevant institution channels are activated before welfare differences are interpreted.
- The activated full run shows successful trade under `none` (`trade_count_mean=5.3252`), active property-right pressure (`property_opportunities_mean=27.8249`), binding trade price controls (`trade_institution_blocked_count_mean=4.7862`, `trade_count_mean=0.0`), and reputation-mediated welfare gains (`welfare_mean=1.3477`).
- The strict-local ablation confirms spatial friction matters: moving from all-island matching to `trade_radius=1` cuts baseline successful trade from `5.3252` to `1.8996`, reputation trade from `6.6665` to `1.9268`, and reputation welfare from `1.3477` to `1.0906`. Property-right pressure and price-control binding still occur under strict-local trade (`property_opportunities_mean=24.4187`; `trade_institution_blocked_count_mean=3.0876`), so institution activation does not depend entirely on centralized matching.
- The full cross-mind ladder shows architecture-dependent trade discovery. PPO and centralized-critic learn the strongest trade/reputation channel (`none` trade around `8.9`/`8.7`; `reputation_system` trade around `14.0`/`7.8`), while DQN and fixed independent-DQN trade less but remain distinct conditions. This is a learner-architecture result under an activated Resource Island configuration, not a claim about an inactive trade channel.

Resource Island activation-hardening queue:
...
- [x] Add unequal exchange offers or price vectors so `trade_price_controls` can actually bind.
...
- [x] Run strict-local trade-radius ablations versus whole-island matching to separate spatial-friction effects from centralized-market effects.
...
- [x] Group-size sweep runner: `run_public_goods_group_size_sweep.py` supports `n_agents in {2, 4, 8, 16}` across Q-learning, DQN, PPO, fixed independent-DQN, centralized-critic, and core Public Goods institutions; local smoke output validated the schema and scaling-effects table.
- [ ] Group-size full run: run the full `n_agents in {2, 4, 8, 16}` sweep to test Olson-style group-size predictions and whether the low-contribution PPO/centralized-critic pattern persists.
...
- Resource Island tabular-Q validation exists, Resource Island Phase 3 torch cross-world wiring passes validation and all-institution smoke tests, and full Resource Island n=20 cross-mind torch runs are complete. Use `outputs/resource_island_phase3_full/mind_comparison.csv` for thesis-facing comparisons.
...
- [x] `tools/theory_scout/`: reproducible literature metadata pipeline exists with OpenAlex, Semantic Scholar, arXiv, optional PDF resolution/download, text extraction, paper-card generation, coverage-first review outputs, ranking, and novelty-gap table construction.
...
- [x] `tools/theory_scout/hydrate_texts.py`: ranked-paper PDF/text hydration stage exists; it resolves open-access PDF URLs from metadata/Unpaywall, downloads PDFs, extracts canonical text files, supports balanced per-world hydration, and writes per-paper status rows to `literature/pdf_text_report.csv`.
- [x] `tools/theory_scout/review_outputs.py`: coverage-first review writer exists and generates `literature/theory_coverage.csv`, `literature/theory_coverage.md`, `literature/manual_pdf_queue.csv`, and `literature/scholar_comparison_worksheet.csv` so every world has theory/PDF/Scholar review rows independent of rank.
- [x] `scripts/run_theory_scout_overnight_ofi1.sh`: one-command overnight pipeline exists for metadata search, balanced PDF/text hydration, balanced local-LLM card filling through `ofi1`, obligation table rebuilds, coverage/manual-PDF/Scholar review outputs, and deterministic obligation audit.
...
- [x] `tools/theory_scout/theory_code_audit.py`: theory-to-code decision audit exists and writes `literature/theory_code_audit.csv` plus `literature/theory_code_audit.md`, mapping each world obligation to required code evidence, output evidence, missing columns/protocol gates, needed fixes, rerun requirements, obsolete outputs, and safe claim boundaries.
- [x] `tools/theory_scout/citation_audit.py`: citation and claim-support audit exists, reads `paper/main.tex` plus `paper/references.bib`, classifies theory/method/result/limitation claims, checks existing BibTeX keys, suggests canonical citations from the foundation-paper key set, maps result claims to output artifacts where possible, and writes `paper/citation_audit.csv`, `paper/citation_audit.md`, and `paper/main_citation_patches.md`.
- [x] Paper citation/framing pass: `paper/main.tex` now uses the learner-architecture / assumption-violation framing instead of a scalar capability ladder, cites the DQN/PPO/independent-learning/centralized-critic method anchors, cites learned-auction design in the Auction House theory section, and builds successfully to `paper/main.pdf`; the current citation audit has zero `needs_citation` rows, with remaining work limited to artifact links and manual judgment rows.
- [x] Theory-engine validation observed: `test_theory_scout.py` runs 31 tests, `test_citation_audit.py` runs 4 tests, compile check passes for `tools/theory_scout`, a real PDF hydration smoke downloaded/extracted `Optimal-er Auctions through Attention` (`text_chars=70209`), one real full-text `ofi1` LLM card-fill smoke succeeded, balanced per-world selection is unit-tested, `review` writes coverage/manual-PDF/Scholar worklists from the current cache, `theory-code-audit` currently reports `pass=10`, `partial=0`, `missing=0`, and `citation-audit` writes the current paper citation/artifact review queue.
...
- [ ] Manually review `literature/scholar_comparison_worksheet.csv` against Google Scholar for each query and record Scholar-only papers, API false positives, and missing canonical theory anchors.

Theory Engine note:

- The current system now finds, structures, hydrates source text, fills cards, audits theory obligations, and produces balanced per-world coverage/manual-review worklists. It is still not a finished deep-reading system: most non-auction paper cards remain templates, PDF/text extraction has only sparse full-text coverage locally, and filled cards require human review before citation. Ranking is treated as secondary; the thesis-facing requirement is that each world has classical theory, known RL/MARL results, benchmark obligations, metrics, and documented gaps.

## Phase 7 - Visualization Layer

- [x] Static web bundle exporter exists at `scripts/export_web_bundle.py`, producing a browser-ready Pricing Arena scenario from validated offline traces, DQN budget-ladder audit rows, paired-seed deltas, full comparison metrics, and a separate all-world ladder bundle from the five thesis-facing mind-comparison tables.
- [x] Vite + React + PixiJS canvas app exists under `explorer/`, with a five-world / five-learner dashboard plus a Primer-style vertical slice for "Can a price cap increase firm profits?"
- [x] All-world visualization bundle exists at `explorer/public/data/all-world-ladder/summary.json`, covering Pricing Arena, Resource Island, Auction House, Public Goods, and Labor Market across Q-learning, DQN, PPO, fixed independent-DQN, and centralized-critic where the full-run tables support them.
- [x] Cross-world dashboard exists in `explorer/src/components/CrossWorldDashboard.tsx`, exposing per-world institution selectors, metric selectors, all-mind bar comparisons, and source artifact links so PPO/MARL results are visible rather than buried behind the Pricing/DQN trace.
- [x] Pricing Arena replay view exists: side-by-side no-regulation vs. price-cap animation, learner selector, playback controls, live metrics, price-cap clipping events, margin/quantity/profit decomposition, budget-ladder chart, and paired-seed distribution.
- [ ] `AgentInspector`
- [x] `MetricsDashboard` first slice exists for Pricing Arena price-cap reversal: price, profit, quantity, welfare, cap-bound events, and seed/budget diagnostics.
- [x] `ReplayPlayer` first slice exists for precomputed Pricing Arena traces.
- [x] GitHub Pages deployment workflow exists at `.github/workflows/pages.yml`, building `explorer/dist` from regenerated static data.
- [ ] Public demo deploy: enable GitHub Pages on the canonical repo and verify the deployed URL.
...
- [x] Initial Pricing Arena answer: price cap's collusion effect is capability- and metric-sensitive; DQN sits near the cap with low price dispersion and retains higher profit-normalized collusion than PPO/random, so price-based and profit-based collusion tell different stories for stronger learners. The 1000-step mechanism trace contradicts the 40k sign, and the dedicated paired-seed audit resolves that as a budget-dependent transition: DQN cap-minus-none profit is negative at 1000 steps (`mean=-11.710856`), mixed at 5000 (`mean=1.730454`, positive share `0.50`), mostly positive by 10000 (`mean=35.447577`, positive share `0.80`), and positive in the existing 40k full table (`mean=55.357821`, positive share `0.85`). Fixed independent-DQN is now a distinct corroborating condition: it remains near the cap under price-cap regulation, with lower profit than plain DQN but higher profit-normalized collusion than PPO.
- [x] Resource Island answer: in the pressure-tested scarce-resource configuration, property/trade/price-control channels are exercised before welfare differences are interpreted. Successful trade appears under `none`, property-right pressure is measurable, trade price controls bind and suppress exchange, and reputation increases trade/welfare under the tested settings.
- [x] Resource Island cross-mind answer: PPO and centralized-critic activate the trade/reputation channel strongly, while DQN and fixed independent-DQN trade less but remain distinct conditions. This is an activation-conditioned learner-architecture result, not a claim about an inactive trade channel.
- [x] Initial cross-world synthesis: capability effects are real in both worlds, but their form is world-specific. In Pricing Arena, stronger learners can exploit a price cap after sufficient training, but the dedicated audit shows the effect is budget-dependent and seed-heterogeneous rather than visible in short traces as a simple quantity channel. In Resource Island, stronger learners differ in whether they discover the activated trade/reputation channel, so the non-pricing result should be framed as architecture-dependent exploration/coordination behavior under a pressure-tested environment.
- [x] Full-data five-world synthesis: `outputs/cross_world_synthesis/` now combines comparable full ladder outputs for Pricing Arena, Resource Island, Auction House, Public Goods, and Labor Market. The protocol-comparability report passes for all five worlds, and `outputs/publication_inference/` has been rebuilt with all five worlds included in confirmatory inference.
- [x] Cross-world claim-strength audit layer: `run_claim_audit_suite.py` now detects paired-seed robustness classes, metric conflicts, activation confirmations, and trace/full-run sign mismatches across the five full-run worlds. Current outputs flag the Pricing Arena DQN/independent-DQN/centralized-critic price-cap trace/full-run profit mismatches, Public Goods reward/accounting warnings, Labor Market welfare-vs-truthfulness/stability tradeoffs, Auction revenue-vs-efficiency/regret tradeoffs, and Resource Island activation confirmations.
- [ ] Revisor synthesis layer: replace scalar `capability_ladder` interpretation with learner-assumption / architecture-axis reports, including architecture-property labels and claim-strength grouping in the generated cross-world synthesis tables. The prose draft has been reframed; the synthesis builder still needs schema-level learner-axis labels.
- [x] Public Goods group-size robustness sweep machinery: `run_public_goods_group_size_sweep.py` runs `n_agents` x mind x institution sweeps and writes a scaling-effects table; local smoke output covered `n_agents={2,3}` for baseline and contribution matching.
- [ ] Public Goods group-size robustness full run: run `n_agents in {2, 4, 8, 16}` across core minds/institutions to test Olson-style group-size/free-riding predictions and whether the low-contribution PPO/centralized-critic pattern persists.
- [ ] Post-run mechanism diagnostics: after the active group-size and N-firm sweeps finish, run `scripts/run_post_run_diagnostics.sh` to regenerate known-answer checks and mechanism traces for writing/revisor evidence.
- [x] Pricing Arena N-firm multiseed training support: `run_multiseed.py --n-firms` runs N-firm Pricing Arena training/evaluation summaries, with 3-firm random smoke output observed locally and focused tests covering a 3-firm world step and runner output.
- [ ] Pricing Arena N-firm boundary sweep: run `n_firms in {2, 3, 4, 5}` for `none` and `price_cap` to test whether the price-cap profit channel survives more competitors.
- [x] Pricing Arena N-firm exploitability protocol: `run_exploitability.py --n-firms --adversary-index` now implements one-deviator exploitability by freezing N-1 incumbent policies and replacing one target firm with a fresh tabular adversary; a 3-firm local smoke validates metadata and CSV schema.
- [ ] Pricing Arena N-firm exploitability full sweep: run the one-deviator protocol over `n_firms in {2, 3, 4, 5}` for `none` and `price_cap`; do not interpret it as coalition exploitability.
