...
and a simple ascending-clock abstraction; the second-price format also supports
a reserve-price variant. Information variants modify bidder observations through
a public signal or bounded value-bin noise, but they do not change the allocation
or payment rule.

This world asks whether learned bidders recover the benchmark behavior that
auction theory predicts in clean mechanisms. The main economic quantities are
seller revenue, bidder surplus, realized welfare, allocative efficiency,
ex-post regret, overbidding, underbidding, truthful-bid distance, and distance
from the first-price shading benchmark.

\subsection{Classical Benchmark and Known-Answer Validation}

Auction House is a first-tier benchmark world because the relevant classical
predictions are exact. In a private-values second-price auction, truthful
bidding is weakly dominant \citep{vickrey1961}. In a symmetric first-price
auction with risk-neutral bidders and independent private values, equilibrium
bids are shaded below value \citep{milgrom2004}. A reserve price changes the
revenue-efficiency tradeoff: it can raise seller revenue by withholding low bids,
but it can also lower realized welfare by preventing efficient sales
\citep{myerson1981}. The clock abstraction shares the second-price reference in
this implementation: dropout at value is the truthful benchmark.

The known-answer layer checks these claims before learned behavior is
interpreted. The benchmark runner verifies that truthful bidding has zero grid
regret in second-price auctions for both two and three bidders. Unit tests cover
allocation, first-price payment, second-price payment, reserve/no-sale behavior,
clock clearing, deterministic tie-breaking, information-policy observation
transforms, truthful second-price benchmarks, first-price bid-shading
benchmarks, ex-post regret, and Q-learning training. The validation obligation
is therefore direct: learned bidders should be compared with the exact benchmark
behavior, not evaluated by payoff alone.

\subsection{Sealed-Bid Q-Learning Results}

Table~\ref{tab:auction-qlearning} shows the full \(n=20\) Q-learning run for the
three sealed-bid scenarios. The world reproduces the intended economic
structure, but learned bidders do not fully recover the theoretical benchmark.
Second-price auctions remain below perfect allocative efficiency, at \(0.734\)
instead of the truthful benchmark value of \(1.0\). First-price auctions show
the expected bid-shading direction: underbidding is frequent, at \(0.814\), and
overbidding is rare, at \(0.066\). The reserve-price variant raises seller
revenue from \(3.100\) in no-reserve second-price auctions to \(3.938\), while
lowering total realized welfare from \(6.241\) to \(6.060\) and producing a
positive no-sale rate.
...
The first-price result is the cleanest recovery of a theory-predicted direction:
learned bidders shade bids below value. The second-price result is the opposite
kind of evidence. The mechanism has a dominant-strategy benchmark, but the
learned bidders do not reach it perfectly in finite training; the consequence is
lower allocative efficiency and positive regret. The reserve result follows the
standard revenue-efficiency interpretation: seller revenue rises while realized
welfare falls relative to the no-reserve second-price baseline.

\subsection{Cross-Mind Sealed-Bid Results}

Table~\ref{tab:auction-crossmind-second} focuses on second-price auctions,
where the benchmark is strongest. PPO is closest to the intended outcome among
the tested learning minds: it has the lowest ex-post regret, \(0.187\), the
highest welfare, \(6.389\), and the highest allocative efficiency, \(0.762\).
DQN and independent-DQN raise seller revenue but do so with lower efficiency and
larger truthful-bid distances. The centralized critic has substantially weaker
results in this asymmetric bidding setting: revenue is nearly zero, allocative efficiency is
\(0.522\), and regret is \(1.436\).
...
underbidding, with underbid rates from \(0.716\) to \(0.814\), consistent with
bid shading. None of them exactly recover the grid benchmark, but the direction
of deviation is economically coherent. Centralized critic again has the weakest
auction behavior, with lower revenue, lower efficiency, and the highest regret.
...
The reserve-price variant is the most direct institutional intervention in this
world. Across several learners, reserve prices increase seller revenue relative
to ordinary second-price auctions while reducing bidder surplus. For Q-learning,
revenue rises from \(3.100\) to \(3.938\), bidder surplus falls from \(3.141\)
to \(2.122\), and no-sale rate rises to \(0.172\). DQN and independent-DQN show
the same revenue shift, with reserve revenues above \(4.3\). This is the
expected reserve-price tradeoff rather than a new theoretical claim.

Clock and information variants are useful diagnostic extensions. PPO remains
strong in the clock variant, with welfare \(6.402\), efficiency \(0.769\), and
regret \(0.186\), close to its second-price results. Public-signal and
noisy-signal variants preserve finite behavior for the neural and MARL minds,
but public signals do not automatically improve allocation. For DQN and
independent-DQN, public-signal second-price auctions have lower allocative
efficiency than the corresponding private-signal second-price runs. These
variants therefore support a narrower claim: information changes can be wired
and measured through the same auction interface, but their current results are
not a final information-design conclusion.

\subsection{Relation to the Main Question}

Auction House answers the main thesis question by separating mechanism validity
from learned recovery of the mechanism's guarantee. The classical guarantees
survive at the mechanism layer: second-price truthfulness, first-price shading,
reserve/no-sale behavior, and allocation/payment rules are implemented and
validated against exact checks. They do not fully survive as learned behavior.
Even in a clean second-price auction, learned bidders do not converge all the
way to truthful, efficient bidding in the observed finite runs. First-price
learners recover the qualitative shading direction, but not a zero-regret
benchmark. Reserve prices reproduce the expected revenue-efficiency tradeoff.

The break depends on learning architecture. PPO is closest to the second-price
benchmark among the tested learners, while DQN and independent-DQN generate
higher revenue with weaker allocative efficiency. Centralized critic has the
largest benchmark deviation in this asymmetric auction setting, with weak
results in both second-price and first-price formats. The correct claim is therefore not
that auction theory fails. The claim is that exact auction-theory properties can
be present in the mechanism while finite learned behavior remains far from the
benchmark, and the distance from the benchmark is architecture-dependent.
