...
local OUT = getenv("STAGE2D_CONTRACT_OUT", getenv("OUT", "audit_outputs/stage2d_resize_source") .. "/lua_contract.json")
local ORACLE_DIR = getenv("STAGE2D_ORACLE_DIR", getenv("OUT", "audit_outputs/stage2d_resize_source") .. "/oracle_outputs")
local FIXTURE_PATHS = getenv("STAGE2D_FIXTURE_PATHS", getenv("ATARI_DIR", ".") .. "/audit_outputs/stage2c_resize/fixtures/fixture_paths.txt")
...
local function tensor_strides(tensor)
  local strides = {}
  for dim = 1, tensor:dim() do strides[#strides + 1] = tensor:stride(dim) end
  return strides
end

local function tensor_is_contiguous(tensor)
  local ok, value = pcall(function() return tensor:isContiguous() end)
...
local function numeric_stats(tensor)
  ...
  return {
    ...
    contiguous = tensor_is_contiguous(tensor),
...
local function byte_stats(tensor)
  ...
  return {
    ...
    strides = tensor_strides(byte),
    contiguous = tensor_is_contiguous(byte),
    min = tonumber(numeric:min()),
    max = tonumber(numeric:max()),
    mean = mean,
    std = math.sqrt(tonumber(numeric:clone():add(-mean):pow(2):mean())),
...
local function byte_values(tensor)
  local byte = tensor:contiguous()
  if torch.type(byte) ~= "torch.ByteTensor" then
    byte = byte:clone():clamp(0, 255):byte():contiguous()
  end
  local flat = byte:view(byte:nElement())
  local values = {}
  for i = 1, flat:nElement() do values[#values + 1] = tonumber(flat[i]) end
  return values
end
...
local function squeeze_single_channel(tensor)
  local out = tensor
  if out:dim() == 3 and out:size(1) == 1 then out = out[1] end
  return out:contiguous()
end

local function output_to_byte(representation_name, resized)
  local squeezed = squeeze_single_channel(resized)
  if representation_name == "float_0_1" then
    return squeezed:clone():mul(255):clamp(0, 255):byte():contiguous()
  end
  return squeezed:clone():clamp(0, 255):byte():contiguous()
end

local function input_representations(input_byte)
  local h = input_byte:size(1)
  local w = input_byte:size(2)
  return {
    {
      name = "byte_0_255",
      actual_net_downsample_representation = false,
      tensor = input_byte:view(1, h, w):contiguous(),
      final_cast = "clamp(0,255):byte()",
    },
    {
      name = "float_0_255",
      actual_net_downsample_representation = false,
      tensor = input_byte:float():view(1, h, w):contiguous(),
      final_cast = "clamp(0,255):byte()",
    },
    {
      name = "float_0_1",
      actual_net_downsample_representation = true,
      tensor = input_byte:float():div(255):view(1, h, w):contiguous(),
...
for _, fixture in ipairs(read_fixture_paths(FIXTURE_PATHS)) do
  ...
  for _, representation in ipairs(input_representations(input_byte)) do
    local record = {
      phase = "stage2d_resize_contract",
      source = "deepmind",
      fixture_name = fixture.name,
      fixture_group = fixture.group,
      input_path = fixture.path,
      input_representation = representation.name,
      actual_net_downsample_representation = representation.actual_net_downsample_representation,
      input_tensor = numeric_stats(representation.tensor),
      resize_call = "image.scale(input_tensor, 84, 84, 'bilinear')",
      final_cast = representation.final_cast,
    }
    local ok, resized = pcall(function()
      return image.scale(representation.tensor, 84, 84, "bilinear")
    end)
    if ok then
      local squeezed = squeeze_single_channel(resized)
      local byte = output_to_byte(representation.name, squeezed)
      local output_dir = ORACLE_DIR .. "/" .. representation.name
      local output_path = output_dir .. "/" .. fixture.name .. ".npy"
      write_npy_uint8(output_path, byte)
      record.output_path = output_path
      record.output_tensor = numeric_stats(squeezed)
      record.output_frame = byte_stats(byte)
      record.output_values = byte_values(byte)
    else
      record.error = tostring(resized)
    end
